...
export default function PortfolioScreen() {
  ...
  useEffect(() => {
    const loadPortfolio = async () => {
      ...
      try {
        ...
        const portRes = await fetch(`${BACKEND_URL}/api/portfolio/graduate/${graduateId}/portfolio`, { credentials: 'include', headers: { Authorization: `Bearer ${token}` } });
        if (portRes.status === 404) { setPortfolio(null); } else if (!portRes.ok) { const data = await portRes.json().catch(() => ({})); throw new Error(data.message || data.error || 'Failed to load portfolio'); } else { const data: Portfolio = await portRes.json(); setPortfolio(data); }
        ...
        if (certRes.ok) { const certs = await certRes.json(); setCertificates(Array.isArray(certs) ? certs : []); } else if (certRes.status !== 404) { const data = await certRes.json().catch(() => ({})); console.log('Certificates load error:', data); }
        setAuthToken(token);
  ...
  const handleDeletePortfolio = async () => {
    Alert.alert('Delete Portfolio', 'Are you sure you want to delete this portfolio? This action cannot be undone.', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        if (!graduateId || !authToken) { setError('Missing authentication or graduate ID.'); return; }
        try {
          const res = await fetch(`${BACKEND_URL}/api/portfolio/graduate/${graduateId}/portfolio`, { method: 'DELETE', headers: { Authorization: `Bearer ${authToken}` }, credentials: 'include' });
          if (!res.ok) { const data = await res.json().catch(() => ({})); throw new Error(data.message || data.error || 'Failed to delete portfolio'); }
  ...
  const handleCertificatePress = (certificate: any) => { setSelectedCertificate(certificate); };
  ...
  return (
    <View style={{ flex: 1, padding: 16, backgroundColor: 'white' }}>
        ...
        <ScrollView>
          <View style={styles.portfolioHeader}>
            ...
            <View style={styles.actionButtons}>
              <TouchableOpacity style={styles.editButton} onPress={() => router.push('/createportfolio')}>
          ...
          {graduate?.profilePicture ? (<Image source={{ uri: graduate.profilePicture }} style={{ width: 120, height: 120, borderRadius: 60, marginBottom: 12 }} />) : null}
          ...
          <Text>Skills</Text>
          {(portfolio.skills || []).map((s, idx) => (<Text key={`skill-${s.id || idx}`}>{s.name} {s.type ? `(${s.type})` : ''} {s.proficiencyLevel ? `- ${s.proficiencyLevel}` : ''}</Text>))}
            ...
            {certificates.length > 0 ? (
              <View style={styles.certificateGrid}>
                {certificates.map((certificate, idx) => (
                  <TouchableOpacity key={`cert-${certificate.id || idx}`} style={styles.certificateItem} onPress={() => setSelectedCertificate(certificate)}>
                    {certificate.certificateFilePath ? (
                      <Image source={{ uri: certificate.certificateFilePath }} style={styles.certificateThumbnail} resizeMode="cover" />
                    ) : (
                      <View style={styles.certificatePlaceholder}><Text style={styles.placeholderText}>📄</Text></View>
                    )}
                    <Text style={styles.certificateTitle} numberOfLines={2}>{certificate.courseName || 'Unnamed Certificate'}</Text>
                    <Text style={styles.certificateNumber} numberOfLines={1}>{certificate.certificateNumber || 'No number'}</Text>
                    <Text style={styles.certificateDate} numberOfLines={1}>{certificate.issueDate || 'No date'}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            ) : (
      ...
      <Modal visible={selectedCertificate !== null} transparent={true} animationType="fade" onRequestClose={closeCertificateModal}>
          ...
          <View style={styles.modalContent}>
            ...
            {selectedCertificate && (
              <View style={styles.certificateModalContent}>
                <Text style={styles.modalTitle}>{selectedCertificate.courseName || 'Certificate'}</Text>
                <Text style={styles.modalSubtitle}>{selectedCertificate.certificateNumber || 'No certificate number'}</Text>
                <Text style={styles.modalDate}>Issued: {selectedCertificate.issueDate || 'No date'}</Text>
                {selectedCertificate.certificateFilePath ? (
                  <Image source={{ uri: selectedCertificate.certificateFilePath }} style={styles.modalImage} resizeMode="contain" />
                ) : (
...
const styles = StyleSheet.create({
  ...
  editButton: { backgroundColor: '#28a745', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 6 },
  editButtonText: { color: 'white', fontSize: 14, fontWeight: '600' },
  deleteButton: { backgroundColor: '#dc3545', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 6 },
  deleteButtonText: { color: 'white', fontSize: 14, fontWeight: '600' },
  sectionContainer: { marginVertical: 16, padding: 16, backgroundColor: '#f8f9fa', borderRadius: 8, borderLeftWidth: 4, borderLeftColor: '#007bff' },
  sectionTitle: { fontSize: 18, fontWeight: 'bold', color: '#333', marginBottom: 16 },
  certificateGrid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' },
  certificateItem: { width: (screenWidth - 80) / 2, backgroundColor: 'white', borderRadius: 8, padding: 12, marginBottom: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 },
  certificateThumbnail: { width: '100%', height: 120, borderRadius: 6, marginBottom: 8 },
  certificatePlaceholder: { width: '100%', height: 120, backgroundColor: '#e9ecef', borderRadius: 6, justifyContent: 'center', alignItems: 'center', marginBottom: 8 },
  placeholderText: { fontSize: 32, color: '#6c757d' },
  certificateTitle: { fontSize: 14, fontWeight: '600', color: '#333', marginBottom: 4 },
  certificateNumber: { fontSize: 12, color: '#666', marginBottom: 2 },
  certificateDate: { fontSize: 12, color: '#666' },
  noDataText: { textAlign: 'center', color: '#666', fontStyle: 'italic', marginVertical: 20 },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0, 0, 0, 0.8)', justifyContent: 'center', alignItems: 'center' },
  modalContent: { width: screenWidth * 0.9, maxHeight: screenWidth * 0.8, backgroundColor: 'white', borderRadius: 12, padding: 20, position: 'relative' },
  closeButton: { position: 'absolute', top: 10, right: 15, width: 30, height: 30, borderRadius: 15, backgroundColor: 'rgba(0, 0, 0, 0.5)', justifyContent: 'center', alignItems: 'center', zIndex: 1 },
  closeButtonText: { color: 'white', fontSize: 18, fontWeight: 'bold' },
  certificateModalContent: { alignItems: 'center' },
  ...
  modalDate: { fontSize: 12, color: '#999', textAlign: 'center', marginBottom: 16 },
  modalImage: { width: '100%', height: screenWidth * 0.6, borderRadius: 8 },
  noImageText: { fontSize: 14, color: '#666', fontStyle: 'italic', textAlign: 'center', marginVertical: 20 },
});
