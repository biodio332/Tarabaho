...
export default function CreatePortfolio() {
  ...
  // Collections (barebones add forms)
  ...
  // Add handlers
  const addSkill = () => {
    if (!skillDraft.name) { setError('Skill name is required.'); return; }
    setSkills((prev) => [...prev, { name: skillDraft.name, type: skillDraft.type, proficiencyLevel: skillDraft.proficiencyLevel }]);
    setSkillDraft({ name: '', type: 'TECHNICAL', proficiencyLevel: '' });
  };

  const addExperience = () => {
    if (!expDraft.jobTitle || !expDraft.company) { setError('Experience needs job title and company.'); return; }
    setExperiences((prev) => [...prev, { ...expDraft }]);
    setExpDraft({ jobTitle: '', company: '', duration: '', responsibilities: '' });
  };

  const addAward = () => {
    if (!awardDraft.title) { setError('Award title is required.'); return; }
    setAwards((prev) => [...prev, { ...awardDraft }]);
    setAwardDraft({ title: '', issuer: '', dateReceived: '' });
  };

  const addEducation = () => {
    if (!eduDraft.courseName) { setError('Course name is required.'); return; }
    setEducations((prev) => [...prev, { ...eduDraft }]);
    setEduDraft({ courseName: '', institution: '', completionDate: '' });
  };

  const addMembership = () => {
    if (!memDraft.organization) { setError('Organization is required.'); return; }
    setMemberships((prev) => [...prev, { ...memDraft }]);
    setMemDraft({ organization: '', membershipType: '', startDate: '' });
  };

  const addReference = () => {
    if (!refDraft.name) { setError('Reference name is required.'); return; }
    setReferences((prev) => [...prev, { ...refDraft }]);
    setRefDraft({ name: '', position: '', company: '', contact: '', email: '' });
  };
  ...
  const uploadCertificate = async (graduateId: number, token: string) => {
    ...
    if (certFile) {
      formData.append('certificateFile', { uri: certFile.uri, name: certFile.name, type: certFile.type } as any);
    }
    const res = await fetch(`${BACKEND_URL}/api/certificate/graduate/${graduateId}`, {
      method: 'POST',
    ...
    if (!res.ok) {
      ...
      throw new Error(data.message || data.error || 'Failed to upload certificate');
    }
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // token: prefer stored authToken, fallback to get-token
      ...
      // Check certificates first (must have at least one)
      const certRes = await fetch(`${BACKEND_URL}/api/certificate/graduate/${graduateId}`, {
        credentials: 'include',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!certRes.ok && certRes.status !== 404) {
        const data = await certRes.json().catch(() => ({}));
        throw new Error(data.message || data.error || 'Failed to verify certificates');
      }
      ...
      if (!hasAtLeastOneCertificate) {
        setError('Graduate must be verified with at least one certificate.');
        return;
      }
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.message || data.error || 'Failed to create portfolio');
      }
      ...
      <ScrollView>
        ...
        <Button title="Upload Certificate" onPress={async () => {
          try {
            ...
            Alert.alert('Error', e instanceof Error ? e.message : 'Failed to upload certificate');
          } finally {
            setLoading(false);
          }
        }} />
